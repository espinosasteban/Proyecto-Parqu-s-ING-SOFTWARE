...
if (verificador.turno == "rojo") {
	...
	// Salida de la cárcel (fichas rojos)
	if (oCarcel.carcelR and global.dobles) {
		if (global.posir[instancia] == 0){
			global.posir[instancia] = 69
			x = cas69.x
			y = cas69.y
	...
	else if (global.lanzado and !global.dobles and global.posir[instancia] != 0) {
		
		// Se comprueba si se escogió dado
		if (global.seleccionado != 0 and global.resultado != 0) {
			llega = global.posir[instancia] + global.resultado - 1	
			for (i = global.posir[instancia]; i <= llega; i++) {
